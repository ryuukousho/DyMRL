...
def train(args):
    ...
    logging.info("History length = {}".format(args.history_len))
    ...
    train_list = split_by_time(dataset.data["train"])
    valid_list = split_by_time(dataset.data["valid"])
    test_list = split_by_time(dataset.data["test"])
    ...
    if args.test:
        ...
        _, rank, filter_rank = optimizer.evaluate(train_list + valid_list, test_list, static_graph, filtered_ans_test, filters, valid_mode=False, epoch=-1)
        test_metrics_raw = compute_metrics(rank)
        test_metrics_filter = compute_metrics(filter_rank)
        logging.info(format_metrics(test_metrics_raw, split="Raw test_" + args.metrics))
        logging.info(format_metrics(test_metrics_filter, split="Time-aware filtering test_" + args.metrics))
    elif args.prompt:
        pass
    else:
        counter = 0
        ...
        for epoch in range(args.max_epochs):
            ...
            loss, snaps_score = optimizer.epoch(train_list, static_graph, epoch=epoch)
            logging.info("\t Epoch {} | average train loss: {:.4f}".format(epoch, loss))
            if math.isnan(loss.item()):
                break
            model.eval()
            valid_loss, ranks, filter_ranks = optimizer.evaluate(train_list, valid_list, static_graph, filtered_ans_valid, filters, epoch=epoch, valid_mode=True)
